...
- You need: *_age-specific rate_trend.csv (from 'CANCER OVER TIME crolling.py')
- Steps
  1. *_age-specific rate_trend.csv 파일을 업로드하고, 결과물을 저장할 위치를 지정한다.
  2. 저장된 데이터를 확인한다.
'''
...
def upload_file():
    root = Tk()
    root.withdraw()
    file_path = askopenfilename(title="Upload *_age-specific rate_trend.csv", filetypes=[("csv", "*.csv")])
    root.destroy()
...
def shortterm(df_sub, term):
    df_new = pd.DataFrame(columns = ['Country', 'Sex', 'Type', 'Age Group', 'Year', 'Age-Specific Rate', 'Standard Error'])
    if df_sub.empty:
        return df_new
        ...
        for sex in sexes:
            for typ in types:
                for age_group in age_groups:
                    df_sub = df[(df['Country'] == country) & (df['Sex'] == sex) & (df['Type'] == typ) & (df['Age Group'] == age_group)]
